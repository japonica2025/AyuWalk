    ...
    private func stickerEditingChrome(
        ...
        ZStack {
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                ...
                .frame(width: max(72, 98 * scale), height: max(38, 44 * scale))
                .rotationEffect(.degrees(rotationDegrees))
            ...
            .frame(width: max(96, 118 * scale), height: max(58, 64 * scale))
        }
    }

    private func editingChromeSize(for scale: Double) -> CGSize {
        CGSize(width: max(120, 142 * scale), height: max(84, 88 * scale))
    }
